...
class CartProductButton extends HTMLElement {
  connectedCallback() {
    ...
    this.parentElement.addEventListener('click', () => {
      const detail = { productID:this.productID};
      window.dispatchEvent(new CustomEvent('cart:add-product', { detail }));
    });
    this._render();
  }

  _render() {
    render(html`<em>${this.constructor.name} for ${this.productID}</em>`, this.parentElement, this);
  }
}
